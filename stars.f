...
**          INERTIA TENSOR
     ...
     &                   +MASAP(JJ)*BASVECCM(IX)*BASVECCM(JY)
       ...
       BASEIGENVAL(1:3)=0.0
       CALL DIAGONALISE(INERTIA4,BASEIGENVAL)
       CALL SORT_EIGEN(BASEIGENVAL,3)
       DO IX=1,3
...
112   FORMAT(2I14,6F14.6,E14.6,F14.6,I14,3F14.6,3F14.6,6E14.6,
     ...
     &               ST_VXCM(I),ST_VYCM(I),ST_VZCM(I)
      ...
      IF (FLAG_WDM.EQ.1) THEN
       WRITE(4) KONTA2
       WRITE(4) (PARTICLES_PER_HALO_ST(J)-MAX_ORIPA_DM,J=1,KONTA2)
      END IF
...
*********************************************************************
     ...
     &            ' (for faster search)'
       ...
       CALL ARGSORT(N_ST,RXPA(N_DM+1:N_DM+N_ST),INDICES)
