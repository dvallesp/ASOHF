...
*     Here U1,U11 are LOCAL !!!!!!!!!!!!!!!!!
      REAL*4 U1(NMAX,NMAY,NMAZ)
      REAL*4 U11(NAMRX,NAMRY,NAMRZ,NPALEV)
*     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      CALL INTERPOLATE_DENSITY_TSC(NX,NY,NZ,NL,NPATCH,
...
c       WRITE(*,*) I,
c     & MINVAL(POT1(1:PATCHNX(I),1:PATCHNY(I),1:PATCHNZ(I),I)),
c     & MAXVAL(POT1(1:PATCHNX(I),1:PATCHNY(I),1:PATCHNZ(I),I))
c      END DO
...
*
       ...
       APOT1(1:N1,1:N2,1:N3,I)=POT1(1:N1,1:N2,1:N3)
       write(*,*) 'ir,ipatch,its,errtot=',ir,i,ii,errtot
